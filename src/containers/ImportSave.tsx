...
class ImportSave extends Component<IProps, IState> {
  ...
  componentDidUpdate() {
    if (this.importing && this.state.country) {
      ...
      if (this.state.army) this.importArmyEnd(countryName, this.state.army)
      else this.state.armies.forEach(army => this.importArmyEnd(countryName, army))
    }
  ...
  getCountryImportString = () => {
    if (this.state.country) {
      if (this.state.army) return 'Import ' + this.state.country.name + ' and ' + this.state.army.name
      else return 'Import ' + this.state.country.name + ' and all armies'
    }
  ...
  getArmyImportString = () => {
    if (this.state.country) {
      if (this.state.army) return 'Import ' + this.state.army.name
      else return 'Import all armies'
    }
    ...
    return (
      <Grid padded>
        ...
        <Grid.Row columns='4'>
          ...
          <Grid.Column>
            <Button onClick={this.importCountry} disabled={!this.state.country}>
              {this.getCountryImportString()}
            </Button>
          </Grid.Column>
          <Grid.Column>
            <Button
              onClick={this.importArmy}
              disabled={!this.state.country || !this.props.countries[this.state.country.name]}
            >
              {this.getArmyImportString()}
            </Button>
          </Grid.Column>
        </Grid.Row>
        <Grid.Row>
    ...
    return (
      <>
        ...
        <Table.Row>
          ...
          <Table.Cell>{sum(country.traditions)}</Table.Cell>
        </Table.Row>
        <Table.Row>
  ...
  selectCountry = (id: string) => {
    ...
    const armies =
      country && country.armies ? excludeMissing(country.armies.map(id => loadArmy(this.state.file, id))) : []
    if (country) this.setState({ country, armies })
  }
  ...
  importCountry = () => {
    ...
    if (country.omen) enableCountrySelection(countryName, SelectionType.Deity, 'omen' + country.omen)
  ...
  importArmy = () => {
    ...
    this.importing = true
    const countryName = this.state.country.name
    if (this.state.army) this.importArmyStart(countryName, this.state.army)
    else this.state.armies.forEach(army => this.importArmyStart(countryName, army))
  }
  ...
  importArmyStart = (countryName: CountryName, army: SaveArmy) => {
    ...
    if (army.mode === Mode.Land) deleteArmy(countryName, ArmyName.Army)
    else deleteArmy(countryName, ArmyName.Navy)
