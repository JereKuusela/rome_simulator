...
const Navigation = () => {
  ...
  const renderModeButton = (mode: Mode) => {
    return (
      <Button active={mode === currentMode} compact icon basic circular size='tiny' onClick={() => dispatch(setMode(mode))}>
  ...
  return (
    <div id='navigation'>
      <Menu>
        ...
        <div id='menu-info'>
          {process.env.REACT_APP_GAME === 'IR' &&
            renderModeButton(Mode.Land)
          }
          {process.env.REACT_APP_GAME === 'IR' &&
            renderModeButton(Mode.Naval)
          }
          <div id='version'><div>Site version 0.8.2</div><div>Game version {process.env.REACT_APP_GAME === 'IR' ? '1.5.3' : '1.30.4'}</div></div>
        </div>
      </Menu>
      <br />
    </div >
