...
if (process.env.REACT_APP_GAME === 'IR') {
  describe('1 vs 1', () => {
    ...
    const infantry = addValues(getUnit(UnitType.LightInfantry), ValuesType.Modifier, 'Initial', [
      [UnitAttribute.Morale, -0.25]
    ])
    const cavalry = addValues(getUnit(UnitType.LightCavalry), ValuesType.Modifier, 'Initial', [
      [UnitAttribute.Morale, -0.2]
    ])
    ...
    it('no modifiers', () => {
      ...
      expected[1].B.front = [[unit.type, 0.98, 2.2272]]
      expected[6].B.front = [[unit.type, 0.88, 1.5998]]
    ...
    it('increased morale damage taken, terrain bonus and discipline', () => {
      ...
      expected[1].B.front = [[unitD.type, 0.964, 2.005]]
    ...
    it('reduced morale damage taken, offense/defense and experience', () => {
      const unitA = addValues(infantry, ValuesType.Base, 'Test', [
        [UnitAttribute.Offense, 0.1],
        [UnitAttribute.Defense, 0.15],
        [UnitAttribute.Experience, 0.0001]
      ])
      const unitD = addValues(infantry, ValuesType.Base, 'Test', [
        [UnitAttribute.Offense, 0.05],
        [UnitAttribute.Defense, 0.05],
        [UnitAttribute.Experience, 0.0004]
      ])
      addToReserveTest(state, SideType.A, [unitA])
      addToReserveTest(state, SideType.B, [unitD])
      ...
      expected[4].A.front = [[unitA.type, 0.92, 1.8416]]

      expected[2].B.front = [[unitD.type, 0.9, 1.638]]
    ...
    it('versus damage and increased morale damage taken', () => {
      ...
      expected[1].A.front = [[unitA.type, 0.965, 2.089]]
      expected[3].A.front = [[unitA.type, 0.901, 1.6944]]

      expected[1].B.front = [[unitD.type, 0.952, 1.86]]
      expected[3].B.front = [[unitD.type, 0.86, 1.0161]]
