...
import { battle, checkFight, setSeed, refreshBattle, RoundState } from '../battle'
...
export const combatReducer = createReducer<AppState>({} as any)
  ...
  .handleAction(refreshBattle, (state, action: ReturnType<typeof refreshBattle>) => {
    ...
    state = { ...state, battle: state.battle.update(action.payload.mode, value => ({ ...value, round: -1, attacker_rounds: List<RoundState>(), defender_rounds: List<RoundState>()})) }
